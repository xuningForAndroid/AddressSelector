...
//        });
    ...
    public void changeTabsState(TabLayout.Tab tab) {
        for (int i = 0; i < mTabLayout.getTabCount(); i++) {
            ...
            if (i == tab.getPosition()) { // 选中状态
                ...
                text.setTextAppearance(R.style.TabItemSelectStyle);
                indicator.setBackgroundColor(mSelectIndicatorColor);
                indicator.setVisibility(View.VISIBLE);
            } else {// 未选中状态
                text.setTextColor(mUnSelectTextColor);
                indicator.setVisibility(View.INVISIBLE);
                text.setTextAppearance(R.style.TabItemUnSelectStyle);
            }
        }

    }
    public void changeTabsState(int tabIndex) {
        for (int i = 0; i < mTabLayout.getTabCount(); i++) {
            View view = mTabLayout.getTabAt(i).getCustomView();
            if (view == null) {
                return;
